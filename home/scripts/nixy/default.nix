  ...
  nixy =
    pkgs.writeShellScriptBin "nixy"
      ...
      ''
        ...
        if [[ $1 == "rebuild" ]];then
          sudo nixos-rebuild switch --flake ${configDirectory}#${hostname} --impure
        elif [[ $1 == "upgrade" ]];then
          ...
          ssh jack -S -C "cd /home/filip/.config/nixos && git pull && sudo -S nixos-rebuild switch --flake ~/.config/nixos#jack"
        elif [[ $1 == "loop" ]];then
          while true; do
            nixy
